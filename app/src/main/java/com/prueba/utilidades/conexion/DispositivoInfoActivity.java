...
public class DispositivoInfoActivity extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(manager.getSimState()==TelephonyManager.SIM_STATE_READY){
			...
			items.add(new ItemTexto("Operador de la SIM", manager.getSimOperator()));
			items.add(new ItemTexto("ID del dispositivo",manager.getDeviceId()));
			items.add(new ItemTexto("Operador de la Red",manager.getNetworkOperator()));
			setContentView(R.layout.layout_main);
			ListView lista=(ListView)findViewById(R.id.lista_actividades);
			lista.setAdapter(new ItemsTextoAdapter(getApplicationContext(), items));
		}else{
